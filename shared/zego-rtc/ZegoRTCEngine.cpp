...
//#include "../../UI/gbs/common/QBizLogger.h"
...
public:
	...
	void LoginAndStartPush(std::string roomId, std::string stream, std::string userId) override;
	void LogoutAndStopPush() override;
...
private:
	...
	std::atomic<bool> need_relogin_{false};
};
...
void ZegoRTCEngineImpl::CreateEngine()
{
	...
	if (engine_ == nullptr) {
		...
		profile.scenario = ZegoScenario::ZEGO_SCENARIO_STANDARD_VOICE_CALL;
	...
	worker_thread_ = std::thread([this]() {
		...
		while (thread_running_) {
			...
			auto execution_time = std::chrono::duration_cast<std::chrono::milliseconds>(
				end_time - start_time + std::chrono::nanoseconds(990));
...
void ZegoRTCEngineImpl::LoginAndStartPush(std::string roomId, std::string stream, std::string userId)
{
	LoginRoom(roomId, userId);
	stream_id_ = stream;
}

void ZegoRTCEngineImpl::LogoutAndStopPush() {
	EndTalk();
	LogoutRoom();
}
				      ...
				      const std::string &extendData)
{
	if (state == ZEGO_ROOM_STATE_DISCONNECTED) {
		logined_ = false;
		//QLogE("connect rtc server disconnected.");
		printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
	} else if (state == ZEGO_ROOM_STATE_CONNECTING) {
		//QLogE("connect rtc server connecting...");
		printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
	} else if (state == ZEGO_ROOM_STATE_CONNECTED) {
		//QLogE("connect rtc server connected.");
		logined_ = true;
		printf("%s %s %d\n", __FILE__, __FUNCTION__, __LINE__);
	}
}
