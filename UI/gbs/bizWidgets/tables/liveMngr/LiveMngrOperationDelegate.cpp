...
void LiveMngrOperationDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (!recordPixmap.isNull()) {
	    painter->drawPixmap(button2Rect.center() - QPoint(recordPixmap.width() / 2, recordPixmap.height() / 2),
				recordPixmap);
    }

    // 绘制按钮3
    if (!sharePixmap.isNull()) {
	    painter->drawPixmap(button3Rect.center() - QPoint(sharePixmap.width() / 2, sharePixmap.height() / 2),
				sharePixmap);
    }
}
